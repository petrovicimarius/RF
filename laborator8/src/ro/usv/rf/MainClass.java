...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			double[][] distance_array = new double[evaluationSet.length][trainingSet.length];
			
			for(int i = 0; i < trainingSet.length; i++)
			{
				for (int j = 0; j < evaluationSet.length; j++)
				{
					distance_array[j][i] = DistanceUtils.getEuclidianGeneralizedDistance(evaluationSet[j], trainingSet[i]);
				}
			}
			
			String output_class_name = null;
			...
			for(int j = 0; j < evaluationSet.length; j++)
			{
				...
				{
					output_class_name = DistanceUtils.getNNClass(kn_cases[i], distance_array[j], trainingSet);
					System.out.println("k= " + kn_cases[i] + " - " + j + " has class: " + evaluationSet[j][numberOfFeatures] + " : calculated class: " + output_class_name + " - "  + (output_class_name.compareTo(evaluationSet[j][numberOfFeatures]) == 0 ? "corect" : "incorect"));
				}
			}
